...
パーソナルコンピュータのユーザインタフェースの類型として最も知られているのが、GUI(Graphical User Interface)とCUI(Character User Interface)である。例えば、本論文の執筆環境は、LinuxのCompizというオーソドックスなGUIに、論文本体の編集画面、文献をまとめたファイルブラウザ、文献を表示するドキュメントビューワーの3つのウインドウからなる。これらをタッチパッドのクリックによって切り替えることができ、タッチパッドの右端を上下になぞると文献のページをめくることができる。

編集画面だけを見るとCUIである。編集画面は仮想端末(文字ベースのテレタイプ端末をGUI上に再現したもの)であり、この上でシェルというコマンドを入力すると結果を返すプログラムが動いている。この上で、論文の執筆、自動組版、バージョンの管理などのコマンドを全て完結することができる。一方仮想端末自体の機能を使う場合もあり、目が疲れてフォントの画面を大きくする場合、GUIの機能を使ってメニューから選択することになる。

この2者の共通点と相違点について、 [Dourish2001]_ は次のようにまとめている。

iPhoneの登場は全く本質的な変化ではない。
